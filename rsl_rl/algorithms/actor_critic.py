...
class AbstractActorCritic(Agent):
    ...
    def export_onnx(self) -> Tuple[torch.nn.Module, torch.Tensor, Dict]:
        ...
        class ONNXActor(torch.nn.Module):
            ...
            def forward(
                self, x: torch.Tensor, hidden_state: Tuple[torch.Tensor, torch.Tensor] = None
            ):
        ...
        if self.actor.recurrent:
            hidden_state = (
                torch.zeros(
                    self.actor._features[0].num_layers, 1, self.actor._features[0].hidden_size
                ),
                torch.zeros(
                    self.actor._features[0].num_layers, 1, self.actor._features[0].hidden_size
                ),
    ...
    def draw_random_actions(
        ...
        data.update(
            {"actor_observations": actor_obs.clone(), "critic_observations": critic_obs.clone()}
        )
    ...
    def process_transition(
        ...
        if "next_actor_observations" in data and "next_critic_observations" in data:
            next_actor_obs, next_critic_obs = (
                data["next_actor_observations"],
                data["next_critic_observations"],
            )
        else:
            next_actor_obs, next_critic_obs = self._process_observations(
                next_observations, next_environment_info
            )
    ...
    def _process_dataset(self, dataset: Dataset) -> Dataset:
        ...
        for idx in range(len(dataset) - self._return_steps + 1):
            ...
            for k in range(self._return_steps):
                ...
                rewards[alive_idx] += (
                    self._discount_factor ** (k + 1)
                    * data["timeouts"][alive_idx]
