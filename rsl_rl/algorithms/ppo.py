...
class PPO(AbstractActorCritic):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._critic_input_size = (
            self._critic_input_size - self._action_size
        )  # We use a state-value function (not Q)
        ...
        self.critic = self.critic_network(
            self._critic_input_size, 1, **self._critic_network_kwargs
        )
        ...
        if self.recurrent:
            ...
            tp = lambda v: v.transpose(
                0, 1
            )  # for storing, transpose (num_layers, batch, F) to (batch, num_layers, F)
    ...
    def update(self, dataset: Dataset) -> Dict[str, Union[float, torch.Tensor]]:
        ...
        for idx, batch in enumerate(
            self.storage.batch_generator(self._batch_count, trajectories=self.recurrent)
        ):
            if self.recurrent:
                transition_obs = batch["actor_observations"].reshape(
                    *batch["actor_observations"].shape[:2], -1
                )
                observations, data = transitions_to_trajectories(transition_obs, batch["dones"])
                hidden_state_h, _ = transitions_to_trajectories(
                    batch["actor_state_h"], batch["dones"]
                )
                hidden_state_c, _ = transitions_to_trajectories(
                    batch["actor_state_c"], batch["dones"]
                )
                ...
                hidden_state = (
                    hidden_state_h[0].transpose(0, 1),
                    hidden_state_c[0].transpose(0, 1),
                )

                action_mean, action_std = self.actor.forward(
                    observations, hidden_state=hidden_state, compute_std=True
                )
                ...
                action_mean, action_std = self.actor.forward(
                    batch["actor_observations"], compute_std=True
                )
            ...
            loss = (
                ...
                - self._entropy_coeff * actions_entropy.mean()
            )
    ...
    def _compute_actor_loss(
        ...
        surrogate_clipped = batch["normalized_advantages"] * ratio.clamp(
            1.0 - self._clip_ratio, 1.0 + self._clip_ratio
        )
    ...
    def _compute_value_loss(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        if self.recurrent:
            observations, data = transitions_to_trajectories(
                batch["critic_observations"], batch["dones"]
            )
            hidden_state_h, _ = transitions_to_trajectories(
                batch["critic_state_h"], batch["dones"]
            )
            hidden_state_c, _ = transitions_to_trajectories(
                batch["critic_state_c"], batch["dones"]
            )
        ...
        value_clipped = batch["values"] + (evaluation - batch["values"]).clamp(
    ...
    def _process_dataset(self, dataset: Dataset) -> Dataset:
        ...
        final_values = self.critic.forward(
            dataset[-1]["next_critic_observations"], **critic_kwargs
        )
        next_values = torch.cat((values[1:], final_values.unsqueeze(0)), dim=0)

        rewards += self.gamma * timeouts * values
        deltas = (rewards + (1 - dones).float() * self.gamma * next_values - values).reshape(
            -1, self.env.num_envs
        )
        ...
        for step in reversed(range(len(dataset))):
            advantages[step] = (
                deltas[step]
                + (1 - dones[step]).float() * self.gamma * self._gae_lambda * advantages[step + 1]
            )
    ...
    def _update_learning_rate(
        ...
        with torch.inference_mode():
            ...
            kl = torch.sum(
                ...
                + (
                    torch.square(batch["actions_std"])
                    + torch.square(batch["actions_mean"] - actions_mean)
                )
